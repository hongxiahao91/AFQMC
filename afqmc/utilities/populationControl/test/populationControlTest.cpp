...
TEST(populationControlTest, capWeight)
{
    size_t L(10);
    vector<double> a(L), a_exact(L);

    a={1,2,3,4,5,6,100,7,8,9};
    a_exact={1,2,3,4,5,6,10,7,8,9};
    capWeight(a, 0.2);
    EXPECT_POINTER_DOUBLE_EQ( a.size(), a.data(), a_exact.data() );

    a={1,50,3,20,5,6,100,7,8,9};
    a_exact={1,14.75,3,14.75,5,6,14.75,7,8,9};
    capWeight(a, 0.2);
    EXPECT_POINTER_DOUBLE_EQ( a.size(), a.data(), a_exact.data() );

    a={1,50,3,20,80,6,100,7,8,9};
    a_exact={1,26,3,20,26,6,26,7,8,9};
    capWeight(a, 0.2);
    EXPECT_POINTER_DOUBLE_EQ( a.size(), a.data(), a_exact.data() );
}
