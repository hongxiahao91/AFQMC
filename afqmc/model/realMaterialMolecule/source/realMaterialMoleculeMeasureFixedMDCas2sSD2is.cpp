...
#include "../include/realMaterialMoleculeMeasureFixedMDCas2sSD2is.h"
#include "../../../utilities/manipulateMCData/include/writeThreadSum.h"

using namespace std;
using namespace tensor_hao;

RealMaterialMoleculeMeasureFixedMDCas2sSD2is::RealMaterialMoleculeMeasureFixedMDCas2sSD2is()
{
    initModelWalkerNullptr();
    reSet();
}

RealMaterialMoleculeMeasureFixedMDCas2sSD2is::RealMaterialMoleculeMeasureFixedMDCas2sSD2is(const RealMaterialMolecule &realMaterialMolecule_, MDCas2s &walkerLeft_)
{
...
void RealMaterialMoleculeMeasureFixedMDCas2sSD2is::initModelWalkerNullptr()
{
    realMaterialMolecule = nullptr;
    walkerLeft = nullptr;
}

void RealMaterialMoleculeMeasureFixedMDCas2sSD2is::setModelWalker(const RealMaterialMolecule &realMaterialMolecule_, MDCas2s &walkerLeft_)
{
...
void RealMaterialMoleculeMeasureFixedMDCas2sSD2is::reSet()
{
...
complex<double> RealMaterialMoleculeMeasureFixedMDCas2sSD2is::returnEnergy()
{
...
TensorHao<double,1> RealMaterialMoleculeMeasureFixedMDCas2sSD2is::returnCholeskyBg()
{
...
void RealMaterialMoleculeMeasureFixedMDCas2sSD2is::addMeasurement(MDCas2sSD2isOperation &mdCas2sSD2isOperation, complex<double> denIncrement)
{
    checkWalkerLeft(mdCas2sSD2isOperation);

    den += denIncrement;

    const SD2is *walkerRight = mdCas2sSD2isOperation.getWalkerRight();
    complex<double> logOverlap = mdCas2sSD2isOperation.returnLogOverlap();
    ...
    addEnergy(mdCas2sSD2isOperation, numIncrement);
}

CholeskyRealForce RealMaterialMoleculeMeasureFixedMDCas2sSD2is::getForce(const CholeskyReal &choleskyReal, MDCas2sSD2isOperation &mdCas2sSD2isOperation, double cap)
{
    checkWalkerLeft(mdCas2sSD2isOperation);
    ...
    const vector< complex<double> > &detUp = mdCas2sSD2isOperation.getDetUp();
    const vector< complex<double> > &detDn = mdCas2sSD2isOperation.getDetDn();
    const vector< TensorHao<complex<double>, 2> > &thetaUpVec = mdCas2sSD2isOperation.returnThetaUp();
    const vector< TensorHao<complex<double>, 2> > &thetaDnVec = mdCas2sSD2isOperation.returnThetaDn();
...
void RealMaterialMoleculeMeasureFixedMDCas2sSD2is::write() const
{
...
double RealMaterialMoleculeMeasureFixedMDCas2sSD2is::getMemory() const
{
    double mem(0.0);
    mem += 8.0*2;
...
RealMaterialMoleculeMeasureFixedMDCas2sSD2is::RealMaterialMoleculeMeasureFixedMDCas2sSD2is(const RealMaterialMoleculeMeasureFixedMDCas2sSD2is &x)
{

}

RealMaterialMoleculeMeasureFixedMDCas2sSD2is & RealMaterialMoleculeMeasureFixedMDCas2sSD2is::operator=(const RealMaterialMoleculeMeasureFixedMDCas2sSD2is &x)
{
    return *this;
}

void RealMaterialMoleculeMeasureFixedMDCas2sSD2is::initWfDaggerT_T()
{
...
void RealMaterialMoleculeMeasureFixedMDCas2sSD2is::initWfDaggerCholeskyVecs_T()
{
    size_t L = walkerLeft->getL(); size_t NupMax = walkerLeft->getNupMax(); size_t NdnMax = walkerLeft->getNdnMax();
    size_t choleskyNumber = realMaterialMolecule->getCholeskyNumber();
...
void RealMaterialMoleculeMeasureFixedMDCas2sSD2is::checkWalkerLeft(const MDCas2sSD2isOperation &mdCas2sSD2isOperation)
{
    if( walkerLeft != mdCas2sSD2isOperation.getWalkerLeft() )
    {
        cout<<"Error!!! RealMaterialMoleculeMeasureFixedMDCas2sSD2is only accept MDCas2sSD2isOperation with fixed MDCas2s!"<<endl;
        exit(1);
    }
}

void RealMaterialMoleculeMeasureFixedMDCas2sSD2is::addEnergy(MDCas2sSD2isOperation &mdCas2sSD2isOperation, std::complex<double> numIncrement)
{
    size_t choleskyNumber = realMaterialMolecule->getCholeskyNumber();
...
complex<double> RealMaterialMoleculeMeasureFixedMDCas2sSD2is::calculateTenergyUp(size_t casIndex, const TensorHao<complex<double>,2> &thetaUp)
{
...
complex<double> RealMaterialMoleculeMeasureFixedMDCas2sSD2is::calculateTenergyDn(size_t casIndex, const TensorHao<complex<double>, 2> &thetaDn)
{
...
TensorHao<complex<double>, 1> RealMaterialMoleculeMeasureFixedMDCas2sSD2is::calculateCholeskyBgUp(size_t casIndex, const TensorHao<complex<double>,2> &thetaUp)
{
    size_t L = walkerLeft->getL(); size_t Nup = walkerLeft->getNup();
    size_t choleskyNumber = realMaterialMolecule->getCholeskyNumber();
    const vector< vector<size_t> > &occupancyUp = walkerLeft->getOccupancyUp();
...
TensorHao<complex<double>, 1> RealMaterialMoleculeMeasureFixedMDCas2sSD2is::calculateCholeskyBgDn(size_t casIndex, const TensorHao<complex<double>, 2> &thetaDn)
{
    size_t L = walkerLeft->getL(); size_t Ndn = walkerLeft->getNdn();
    size_t choleskyNumber = realMaterialMolecule->getCholeskyNumber();
    const vector< vector<size_t> > &occupancyDn = walkerLeft->getOccupancyDn();
...
TensorHao<complex<double>, 1> RealMaterialMoleculeMeasureFixedMDCas2sSD2is::calculateCholeskyExUp(size_t casIndex, const TensorHao<complex<double>,2> &thetaUp)
{
    size_t L = walkerLeft->getL(); size_t Nup = walkerLeft->getNup();
    size_t choleskyNumber = realMaterialMolecule->getCholeskyNumber();
    const vector< vector<size_t> > &occupancyUp = walkerLeft->getOccupancyUp();
...
TensorHao<complex<double>, 1> RealMaterialMoleculeMeasureFixedMDCas2sSD2is::calculateCholeskyExDn(size_t casIndex, const TensorHao<complex<double>,2> &thetaDn)
{
    size_t L = walkerLeft->getL(); size_t Ndn = walkerLeft->getNdn();
    size_t choleskyNumber = realMaterialMolecule->getCholeskyNumber();
    const vector< vector<size_t> > &occupancyDn = walkerLeft->getOccupancyDn();
