...
#include "../include/HubbardKanamoriMeasureFixSDSD.h"
#include "../../../utilities/manipulateMCData/include/writeThreadSum.h"

using namespace std;
using namespace tensor_hao;

HubbardKanamoriMeasureFixSDSD::HubbardKanamoriMeasureFixSDSD()
{
    initModelWalkerNullptr();
    reSet();
}

HubbardKanamoriMeasureFixSDSD::HubbardKanamoriMeasureFixSDSD(const HubbardKanamori &HubbardKanamori_,
                                                             const SD &walkerLeft_)
{
    setModelWalker(HubbardKanamori_, walkerLeft_);
    reSet();
}
...
void HubbardKanamoriMeasureFixSDSD::initModelWalkerNullptr()
{
    hubbardKanamori = nullptr;
    walkerLeft = nullptr;
}

void HubbardKanamoriMeasureFixSDSD::setModelWalker(const HubbardKanamori &HubbardKanamori_, const SD &walkerLeft_)
{
    if( 2*HubbardKanamori_.getL() != walkerLeft_.getL() ) {cout<<"Model L does not consistent with walker L!"<<endl; exit(1);}
    if( HubbardKanamori_.getN() != walkerLeft_.getN() ) {cout<<"Model N does not consistent with walker N!"<<endl; exit(1);}
...
void HubbardKanamoriMeasureFixSDSD::reSet()
{
...
complex<double> HubbardKanamoriMeasureFixSDSD::returnEnergy()
{
...
TensorHao<double, 1> HubbardKanamoriMeasureFixSDSD::returnKanamoriBg()
{
...
void HubbardKanamoriMeasureFixSDSD::addMeasurement(SDSDOperation &sdsdOperation, std::complex<double> denIncrement)
{
...
KanamoriInteractForce HubbardKanamoriMeasureFixSDSD::getForce(const KanamoriInteract &kanamoriInteract,
...
void HubbardKanamoriMeasureFixSDSD::write() const
{
...
double HubbardKanamoriMeasureFixSDSD::getMemory() const
{
    double mem(0.0);
    mem += 8.0;
    mem += 8.0;
...
HubbardKanamoriMeasureFixSDSD::HubbardKanamoriMeasureFixSDSD(const HubbardKanamoriMeasureFixSDSD &x)
{
}

HubbardKanamoriMeasureFixSDSD & HubbardKanamoriMeasureFixSDSD::operator=(const HubbardKanamoriMeasureFixSDSD &x)
{
    return *this;
}

void HubbardKanamoriMeasureFixSDSD::initWfDaggerT()
{
    size_t L = walkerLeft->getL(); size_t N = walkerLeft->getN();

    wfDaggerT.resize(N, L);

    BL_NAME(gmm)(walkerLeft->getWf(), hubbardKanamori->getT(), wfDaggerT, 'C');
}

void HubbardKanamoriMeasureFixSDSD::checkWalkerLeft(const SDSDOperation &sdsdOperation)
{
    if( walkerLeft != sdsdOperation.getWalkerLeft() )
    {
        cout<<"Error!!! HubbardKanamoriMeasureFixSDSD only accept sdsdOperation with fixed SD!"<<endl;
        exit(1);
    }
}

void HubbardKanamoriMeasureFixSDSD::addEnergy(SDSDOperation &sdsdOperation, std::complex<double> denIncrement)
{
    size_t L = hubbardKanamori->getL(); size_t L2 = L*2;  size_t N = walkerLeft->getN();
    size_t numberOfKana = hubbardKanamori->getNumberOfKana();
    const TensorHao<int, 1> &site_i = hubbardKanamori->getSite_i();
    const TensorHao<int, 1> &site_j = hubbardKanamori->getSite_j();
    size_t ki,kj;
...
void HubbardKanamoriMeasureFixSDSD::addKanamoriBg(SDSDOperation &sdsdOperation, complex<double> denIncrement)
{
    size_t L = hubbardKanamori->getL();
    size_t numberOfKana = hubbardKanamori->getNumberOfKana();
    const TensorHao<int, 1> &site_i = hubbardKanamori->getSite_i();
...
void HubbardKanamoriMeasureFixSDSD::addNupNdn(SDSDOperation &sdsdOperation, std::complex<double> denIncrement)
{
    size_t L = hubbardKanamori->getL();
    const TensorHao<complex<double>, 2> &greenMatrix = sdsdOperation.returnGreenMatrix();
...
void HubbardKanamoriMeasureFixSDSD::addSplusSminus(SDSDOperation &sdsdOperation, std::complex<double> denIncrement)
{
    size_t L = hubbardKanamori->getL();
    const TensorHao<complex<double>, 2> &greenMatrix = sdsdOperation.returnGreenMatrix();
