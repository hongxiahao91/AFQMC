...
class MDCas2sSD2sOperationTest: public ::testing::Test
{
 public:
    size_t L, Nup, Ndn;
...
TEST_F( MDCas2sSD2sOperationTest, logOverlap )
{
    MDCas2s mdCas2s(filename);
    SD2s sd2s(L, Nup, Ndn); randomFill( sd2s.wfUpRef() ); randomFill( sd2s.wfDnRef() ); sd2s.logwRef()=complex<double>(2.0, 3.0);

    MDCas2sSD2sOperation walkerWalkerOperation( mdCas2s, sd2s );
    ...
    for(size_t i = 0; i < MDSize; ++i)
    {
        ...
        gmm_cpu( phiTUp, sd2s.getWfUp(), ovlpUp, 'C' );
...
TEST_F( MDCas2sSD2sOperationTest, theta )
{
    ...
    for(size_t i = 0; i < MDUpSize; ++i)
    {
        ...
        gmm_cpu(sd2s.getWfUp(), ovlpUp, thetaUp);
    ...
    for(size_t i = 0; i < MDDnSize; ++i)
    {
        ...
        gmm_cpu(sd2s.getWfDn(), ovlpDn, thetaDn);
