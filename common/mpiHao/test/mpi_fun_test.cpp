...
TEST (MPIBcast, bool_pointer)
{
    const int N = 16;
    bool a[N] = {};
    bool b[N] = {};  for(int i=0; i<N; i++) b[i]=i%2;
    if(MPIRank()==0)
    {
...
TEST (MPIBcast, int_vector)
{
    const size_t N = 16;
    vector<int> a;
    vector<int> b(N);  for(size_t i=0; i<N; i++) b[i]=i*100;
    if(MPIRank()==0)
    {
        a.resize(N);
        for(size_t i=0; i<N; i++) a[i]=b[i];
    ...
    EXPECT_POINTER_EQ( N, b.data(), a.data() );
}

TEST (MPIBcast, size_t_vector)
{
    const size_t N = 16;
    vector<size_t> a;
    vector<size_t> b(N);  for(size_t i=0; i<N; i++) b[i]=i*100;
    if(MPIRank()==0)
    {
        a.resize(N);
        for(size_t i=0; i<N; i++) a[i]=b[i];
...
TEST (MPIBcast, double_vector)
{
    const size_t N = 16;
    vector<double> a;
    vector<double> b(N);  for(size_t i=0; i<N; i++) b[i]=i*100.23;
    if(MPIRank()==0)
    {
        a.resize(N);
        for(size_t i=0; i<N; i++) a[i]=b[i];
    ...
    EXPECT_POINTER_DOUBLE_EQ( N, b.data(), a.data() );
}

TEST (MPIBcast, complexdouble_vector)
{
    const size_t N = 16;
    vector<complex<double>> a;
    vector<complex<double>> b(N);  for(size_t i=0; i<N; i++) b[i]=complex<double>(i*100,i*100);
    if(MPIRank()==0)
    {
        a.resize(N);
        for(size_t i=0; i<N; i++) a[i]=b[i];
    ...
    EXPECT_POINTER_COMPLEXDOUBLE_EQ( N, b.data(), a.data() );
}
