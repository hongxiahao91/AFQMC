...
namespace tensor_hao
{
 ...
 /*******************************/
 /*Calculate determiant with CPU*/
 /*******************************/
 complex<double> determinant_cpu(const TensorCore<complex<double>, 2> &x)
 {
     if( x.rank(0)==1 && x.rank(1)==1 ) return x(0,0);
     else if( x.rank(0)==2 && x.rank(1)==2 ) return x(0,0)*x(1,1)-x(0,1)*x(1,0);
     else return determinant( LUconstruct_cpu(x) );
 }

 complex<double> determinant_cpu(TensorHao<complex<double>, 2> &&x)
 {
     if( x.rank(0)==1 && x.rank(1)==1 ) return x(0,0);
     else if( x.rank(0)==2 && x.rank(1)==2 ) return x(0,0)*x(1,1)-x(0,1)*x(1,0);
     else return determinant( LUconstruct_cpu( move(x) ) );
 }
 ...
 void inverse_cpu(TensorHao<complex<double>, 2> &x)
 {
     if( x.rank(0)==1 && x.rank(1)==1 )
     {
         x(0,0) = 1.0 / x(0,0);
         return;
     }
     else if( x.rank(0)==2 && x.rank(1)==2 )
     {
         complex<double> det = x(0,0)*x(1,1)-x(0,1)*x(1,0);
         complex<double> tmp = x(0,0);
         x(0,0) =  x(1,1)/det; x(0,1) = -x(0,1)/det;
         x(1,0) = -x(1,0)/det; x(1,1) =  tmp/det;
         return;
     }
     else
     {
         x = inverse_cpu( LUconstruct_cpu( move(x) ) );
         return;
     }
 }
 ...
 void QRMatrix_cpu(TensorCore<complex<double>, 2> &ph, TensorCore<complex<double>, 2> &R)
 {
     if( ph.rank(0) < ph.rank(1) ) { cout<<"Error, only deal with the Row>=Column matrix! "<<endl; exit(1); }
     if( ph.rank(1)!= R.rank(0) || R.rank(0)!=R.rank(1) ) { cout<<"R size is not consistent with ph! "<<endl; exit(1); }
     ...
     {
         for (HAO_INT j=0; j<=i; j++) R(j,i) = ph(j,i);
         for (HAO_INT j=i+1; j<N; j++) R(j,i)=0.0;
     }
