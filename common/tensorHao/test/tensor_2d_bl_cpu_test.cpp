...
TEST ( Tensor_2d_bl_cpu, determiant_cpu )
{
    TensorHao<complex<double>, 2> A;
    complex<double> det;

    A.resize(1,1); A(0,0) = complex<double>(1.0, 2.0);
    EXPECT_COMPLEXDOUBLE_EQ( A(0,0), determinant_cpu( move(A) ) );

    A.resize(2,2);
    A(0,0) = complex<double>(1.0, 3.0); A(0,1) = complex<double>(5.0, 2.0);
    A(1,0) = complex<double>(2.0, 2.0); A(1,1) = complex<double>(2.0, 5.0);
    det = complex<double>(-19, -3);
    EXPECT_COMPLEX_NEAR( det, determinant_cpu( move(A) ), 1e-12 );

    A.resize(3,3); randomFill(A);
    det = determinant(LUconstruct_cpu(A));
    EXPECT_COMPLEX_NEAR( det, determinant_cpu( move(A) ), 1e-12 );
}
TEST ( Tensor_2d_bl_cpu, inverse )
{
    ...
    TensorHao<complex<double>, 2> II, II_exact;

    INV_A.resize(1, 1); randomFill(INV_A); A = INV_A;
    II.resize(1, 1); II_exact.resize(1, 1); II_exact(0, 0) = 1.0;
    inverse_cpu(INV_A); gmm_cpu(A, INV_A, II);
    EXPECT_FALSE( diff(II, II_exact, 1e-12) );

    INV_A.resize(2, 2); randomFill(INV_A); A = INV_A;
    II.resize(2, 2); II_exact.resize(2, 2); II_exact=complex<double>(0,0); II_exact(0, 0) = 1.0; II_exact(1, 1) = 1.0;
    inverse_cpu(INV_A); gmm_cpu(A, INV_A, II);
    EXPECT_FALSE( diff(II, II_exact, 1e-12) );

    INV_A.resize(5, 5); randomFill(INV_A); A = INV_A;
    II.resize(5, 5); II_exact.resize(5, 5); II_exact=complex<double>(0,0); for(size_t i = 0; i < 5 ; ++i) II_exact(i, i) = 1.0;
    inverse_cpu(INV_A); gmm_cpu(A, INV_A, II);
    EXPECT_FALSE( diff(II, II_exact, 1e-12) );
}
...
TEST ( Tensor_2d_bl_cpu, QRMatrixRmatrix )
{
    const size_t L0=3; const size_t L1=2;
    TensorHao<complex<double>,2> A(L0,L1), AExact(L0, L1), Q(L0,L1), R(L1, L1) ;

    Q = { {2.0,0.0} ,   {3.0,5.0},    {3.123,3.11},
          {3.0,-6.0},   {2.0,1.0},    {6.123,3.11} };
    ...
    EXPECT_FALSE ( diff( abs (A), abs (AExact), 1e-13) );
}
