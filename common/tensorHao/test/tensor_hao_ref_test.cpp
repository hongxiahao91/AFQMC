...
TEST(Tensor_hao_ref, sliceCopyDeep)
{
    size_t L(4), N(3);

    TensorHao<double,2>  tensor_a(L,N), tensor_b(L,N);
    for(size_t j = 0; j < N; ++j)
    {
        for(size_t i = 0; i < L; ++i) tensor_a(i, j) = i + j*L;
    }

    for(size_t i = 0; i < N; ++i)
    {
        tensor_b[i].copy_deep( tensor_a[i] );
    }

    EXPECT_FALSE( diff( tensor_a, tensor_b, 1e-12 ) );
}